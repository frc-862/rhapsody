...
public class RobotContainer extends LightningContainer{
    ...
    protected void configureButtonBindings() {
        ...
        new Trigger(() -> driver.getPOV() == 0).onTrue(new InstantCommand(() -> pivot.increaseBias(), pivot));
        new Trigger(() -> driver.getPOV() == 180).onTrue(new InstantCommand(() -> pivot.decreaseBias(), pivot));

        new Trigger(() -> driver.getAButton()).onTrue(new Shoot(flywheel, indexer).alongWith(leds.enableState(LED_STATES.SHOOTING)));
        new Trigger(() -> driver.getXButton()).onTrue(new NoteCollect(collector, indexer).andThen(leds.enableState(LED_STATES.COLLECTED)));

        new Trigger(() -> driver.getRightBumper()).onTrue(new InstantCommand(flywheel::cycleShooterSpeed));
    }
    @Override
