...
public class FacingTest extends Command {
    ...
    /**
     * Creates a new FacingTest.
     *
     * @param targetPose the target pose to point at
     * @param drivetrain to request movement
     * @param driver     the driver's controller, used for drive input
     */
    public FacingTest(Translation2d targetPose, Swerve drivetrain, XboxController driver) {
    ...
    private boolean inTolerance() {
        return Math.abs(targetHeading.getDegrees() - drivetrain.getPose().getRotation().getDegrees())
    ...
    public void execute() {
        ...
        double target = Math.toDegrees(Math.atan2(deltaY, deltaX));
        target = (target + 540) % 360;

        targetHeading = new Rotation2d(Math.toRadians(target));

        drivetrain.setFieldFacing(-driver.getLeftY(), -driver.getLeftX(), targetHeading);

        LightningShuffleboard.setDouble("Point-At-Point", "Target Heading", targetHeading.getDegrees());
    ...
    public void updateLogging() {
        ...
        targetHeadingLog.append(targetHeading.getDegrees());
        targetYLog.append(targetPose.getY());
        targetXLog.append(targetPose.getX());
        pidOutputLog.append(pidOutput);
        targetMinusCurrentHeadingLog.append(Math.abs(targetHeading.getDegrees() - drivetrain.getPose().getRotation().getDegrees()));
